/**
 * update-custom-groups.ts
 *
 * Updates the 4 custom model groups to match exact specifications:
 * - qwen-max: all max models EXCEPT qwen-max and qwen3-max (legacy aliases)
 * - qwen-plus: all plus models
 * - qwen-coder: all coder models
 * - qwen-flash: all flash models
 *
 * Usage: npx tsx server/scripts/update-custom-groups.ts
 */

import { getDb, closeDb } from "../lib/database.js";
import { updateGroup, getGroup } from "../lib/group-store.js";
import { createLogger } from "../lib/logger.js";
import type { CandidateModel, ModelCapability } from "../lib/types.js";

const log = createLogger("update-custom-groups");
...
function updateCustomGroup(def: typeof CUSTOM_GROUPS[0]): "created" | "updated" | "skipped" {
  const existing = getGroup(def.id);
  
  if (!existing) {
    log.warn(`  ! group '${def.id}' does not exist - skipping`);
    return "skipped";
  }
  ...
  log.info(`  ✓ updated '${def.id}' with ${def.candidateModels.length} candidates`);
  return "updated";
}
...
function main() {
  ...
  log.info(`Updating ${CUSTOM_GROUPS.length} custom model groups...`);

  let updated = 0;
  let skipped = 0;

  for (const def of CUSTOM_GROUPS) {
    const result = updateCustomGroup(def);
    if (result === "updated") updated++;
    else if (result === "skipped") skipped++;
  }

  log.info(`Done — ${updated} updated, ${skipped} skipped.`);
  log.info("");
  log.info("Group summary:");
  log.info("  - qwen-max:    11 max models (excluding legacy qwen-max, qwen3-max aliases)");
  log.info("  - qwen-plus:   13 plus models");
  log.info("  - qwen-coder:  8 coder models");
  log.info("  - qwen-flash:  18 flash models (text + VL + omni + coder)");
  log.info("");
